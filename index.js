...
]=====> KASIH KREDIT GW ZAKI AJG<=====[
]=====> GANTI YG GW BOLEHIN <=====[
JNG LUPA SUBREK MikazuMZ / IKY ADS/RAMLAN ID 
SPECIAL THANKS TO
RAMLAN ID
BABY BOT
MHANK BAR BAR
...
// KASIH WM GUE
            + 'VERSION:3.0\n' 
// ZAKIGANS UBAH AUTO EROR
            + 'FN:ZAKIGANS \n' 
// JAN NGEYEL UBAH AUTO EROR
            + 'ORG:ZAKIAGANS;\n'
...
const client = new WAConnection()
	...
	client.on('connecting', () => {
		start('2', 'ZAKI Connecting...')
	})
	client.on('open', () => {
		success('2', 'ZAKI Connected')
	})
...
switch(command) {
				...
				if (!isRegistered) return reply(ind.noregis())					
                    ...
                    client.sendMessage(from, tod, image, { quoted: mek, caption: '*Jangan lupa Save No Owner Aku🙈 : ZakiGans*'})
                    break
                case 'soundmenu':
                if (isBanned) return reply(ind.baned())
				if (!isRegistered) return reply(ind.noregis())					
...
*/
				...
				client.sendMessage(from, ''+tes+'\n\n_By : ⸸I AM GANS⸸Panutanque._', text, { quoted: mek })
				await limitAdd(sender)
				break
// NINJALOGO				
				case 'ninjalogo':
...
//GEMBOK TEXT
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					var gem = gh.split("&")[0];
					var bok = gh.split("&")[1];
					if (args.length < 1) return reply('[❗] Contoh : ${prefix}gemboktext 09 02 2006 I AM GANS')
					reply(ind.wait())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					var gli = gh.split("&")[0];
					var tch = gh.split("&")[1];
					if (args.length < 1) return reply('[❗] Contoh : ${prefix}glitchtext ZAKI & Gans')
					reply(ind.wait())
                ...
                var gh = body.slice(12)
					...
					var wm = gh.split("&")[1];
					const pref = `yang mau dijadiin quote apaan, titit?\n\ncontoh : ${prefix}bikinquote aku bukan boneka & Kata ZAKI`
